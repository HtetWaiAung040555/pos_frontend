...
const syncFromCloud = async () => {
    ...
    await usePromo.syncFromCloud(payload);
    if (usePromo.error.length) {
        usePromo.error.forEach((msg) => {
            toast.add({
                severity: 'error',
        ...
        <PageTitle title="Promotion List">
            <template #titleButtons>
                <div class="flex gap-x-2 items-center">
                    <BaseButton icon="fa fa-cloud-download" label="Sync from cloud" severity="primary"
                        @click="syncFromCloud" />
                </div>
            </template>
        </PageTitle>
        <DataTable :columns="columns" :rows="filteredRows" :editPath="'Update Promotion'" :isLoading="usePromo.loading"
            @delete="deleteHandle" :defaultSort="{ key: 'start_at', order: 'desc' }"
            :isEdit="!usePermission.can('Promotion', 'Update')" :isDelete="!usePermission.can('Promotion', 'Delete')">
            <template #filters>
