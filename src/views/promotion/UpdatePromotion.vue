...
function changeRoute(pathname) { router.push(pathname); }
...
onMounted(async () => {
    ...
    if (promoId.value) {
        ...
        formData.value.discountType = promo.discount_type || promo.discountType || 'AMOUNT';
        formData.value.discountValue = promo.discount_value || promo.discountValue || 0;
        ...
        promoStatus.value = String(promo.status_id || promo.status?.id || '1') === '1';

        // populate selectedProducts: promo.products may be objects or ids
        if (Array.isArray(promo.products) && promo.products.length > 0) {
            if (typeof promo.products[0] === 'object') {
                ...
                // array of ids -> map to productList
...
function confirmProductSelection() { selectedProducts.value = selectionBuffer.value.slice(); isProductDialogVisible.value = false; }
...
function getFinalPrice(product) {
    ...
    if (formData.value.discountType === 'AMOUNT') return Math.max(0, price - val);
...
async function formSubmit() {
    if (formData.value.name === "") {
        errorMsg.value = { name: errMsgList.name, discountValue: "", products: "" };
        return;
    } else if (formData.value.discountValue <= 0) {
        errorMsg.value = { name: "", discountValue: errMsgList.discountValue, products: "" };
        return;
    } else if (selectedProducts.value.length === 0) {
        errorMsg.value = { name: "", discountValue: "", products: errMsgList.product };
        return;
    }
    ...
    if (!promoId.value) {
        toast.add({ severity: 'error', summary: 'Missing ID', detail: 'Promotion id is missing.', life: 3000 });
        return;
    }
        ...
        <PageTitle title="Update Promotion">
            <template #titleButtons>
                <div class="flex gap-x-2 items-center">
                    <BaseButton icon="fa fa-chevron-left" label="Back" severity="secondary" @click="changeRoute('/promotion')" />
                </div>
            </template>
        </PageTitle>
        ...
        <BaseCard class="mt-3">
            <template #cardElements>
                ...
                <div class="flex gap-x-4 mt-6">
                    <BaseInput size="sm" v-model="formData.name" label="Name" placeholder="Name" width="300px" height="h-[35px]" :isRequire="true" :error="errorMsg.name" />
                    <div class="flex flex-col gap-y-1 w-[200px]"><BaseLabel label="Status" /><BaseSwitch v-model="promoStatus" /></div>
                </div>
                ...
                <div class="flex gap-x-4 mt-4">
                    <div class="flex flex-col gap-1 w-[300px]"><BaseLabel label="Discount Type" />
                        <select class="text-md border border-gray-500 rounded-sm p-2 text-black w-full h-[35px]" v-model="formData.discountType"><option value="AMOUNT">Amount</option><option value="PERCENTAGE">Percentage</option></select>
                    </div>
                    <BaseInput size="sm" v-model="formData.discountValue" label="Discount Value" width="300px" height="h-[35px]" type="number" />
                </div>
                <div class="flex gap-x-4 mt-4"><BaseInput v-model="formData.description" label="Description" placeholder="Description" /></div>
                ...
                <div v-if="isProductDialogVisible" class="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div class="bg-white rounded shadow-lg w-[90%] max-w-4xl max-h-[80vh] overflow-hidden z-10 p-4">
                        <div class="flex items-center justify-between py-4 border-b"><SubTitle label="Select Products" /><div class="text-sm text-gray-600">{{ selectionBuffer.length }} selected</div></div>
                        <div class="py-4 flex gap-x-2 items-center"><input v-model="searchTerm" placeholder="Search by name or barcode" class="border p-2 rounded w-full" /></div>
                        <div class="py-4 overflow-auto max-h-[50vh]"><table class="w-full text-sm"><thead><tr class="text-left text-gray-600 border-b"><th class="py-2"><div class="flex items-center gap-x-2"><span v-if="isSelectAllLoading" class="text-sm text-gray-600"><i class="fa fa-spinner fa-spin"></i></span><input v-else type="checkbox" :checked="allFilteredSelected" @change="toggleHeaderSelection" ref="headerCheckboxRef" :disabled="isCheckingAll || isSelectAllLoading" /></div></th><th>Image</th><th class="py-2">Name</th><th class="py-2">Barcode</th><th class="py-2 text-end">Price</th></tr></thead>
                                <tbody>
                                    ...
                                    <tr v-if="(filteredProducts || []).length === 0"><td colspan="4" class="py-4 text-center text-gray-500">No products found</td></tr>
                                </tbody></table></div>
                        <div class="flex justify-end gap-x-2 py-4 border-t"><BaseButton severity="secondary" label="Cancel" @click="cancelProductSelection" /><BaseButton label="Add Product" class="px-4 py-2 bg-blue-600 text-white rounded" @click="confirmProductSelection" /></div>
                    </div>
                </div>
