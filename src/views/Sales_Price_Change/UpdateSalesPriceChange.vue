...
const promotionConflictTargets = computed(() => buildPromotionConflictTargets(selectedRows.value));
...
function validateForm() {
    ...
    const rowError = validatePriceChangeRows(selectedRows.value);
    if (rowError) {
...
async function formSubmit() {
    ...
    const result = await runPromotionConflictCheck(
        buildPromotionConflictTargets(selectedRows.value),
        { notifyOnError: true },
    );
    if (!result) return;
