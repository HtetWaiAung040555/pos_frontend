        ...
        <BaseCard class="mt-3">
            <template #cardElements>
                ...
                <div class="flex gap-x-4 mt-6">
                    <!-- Role name input -->
                    <BaseInput size="sm" v-model="formData.name" label="Name" placeholder="Role Name" width="300px"
                ...
                <div class="flex gap-x-4 mt-4 pb-4 border-b border-b-gray-200">
                    <!-- Role desc input -->
                    <BaseTextarea v-model="formData.desc" label="Description" placeholder="" autoResize />
                </div>
                <div class="mt-4 mb-2 flex items-center gap-x-2">
                    ...
                    <!-- Select all checkbox -->
                    <BaseCheckbox :label="isAllSelectedGlobal ? 'Deselect All' : 'Select All'"
                        :model-value="isAllSelectedGlobal" @update:modelValue="checked => toggleAllGlobal(checked)" />
                </div>
                ...
                <div v-else class="mt-4 space-y-2 w-[700px]">
                    <div v-for="(actions, groupName) in groupedPermissions" :key="groupName"
                        ...
                        <div class="flex justify-between items-center">
                            <div class="flex items-center gap-x-4">
                                ...
                                <BaseCheckbox :label="isAllSelected(groupName) ? 'Deselect All' : 'Select All'"
                                    :model-value="isAllSelected(groupName)"
                                    @update:modelValue="checked => toggleAllGroup(groupName, checked)" />
                            </div>
                            <i :class="collapsed[groupName] ? 'fa fa-chevron-down' : 'fa fa-chevron-right'"
                                class="cursor-pointer" @click="toggleCollapse(groupName)">
                            </i>
                        </div>
                        <!-- Collapsible actions -->
                        <div v-if="collapsed[groupName]" class="flex mt-3 ml-4 gap-x-10">
                            <!-- Individual permissions -->
                            <BaseCheckbox v-for="perm in actions" :key="perm.id" :label="perm.action"
                ...
                <div class="flex justify-end mt-4">
                    <!-- Save button -->
                    <BaseButton label="Update" :isLoading="useRole.loading"
                        :icon="useRole.loading ? 'fa fa-spinner' : 'fa fa-floppy-disk'" severity="primary"
                        @click="formSubmit" :disabled="useRole.loading" />
                </div>
            </template>
        </BaseCard>
    </div>
