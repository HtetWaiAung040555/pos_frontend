...
onMounted(async () => {
  ...
  if (priceChangeId.value) {
    ...
    selectedProducts.value = usePriceChange.priceChangeList.products.map(p => {
        ...
        old_purchase_price: Number(p.old_purchase_price) || 0,
        new_purchase_price: Number(p.purchase_price),
...
function confirmProductSelection() {
  selectedProducts.value = selectionBuffer.value.map(p => ({
    ...p,
    old_purchase_price: Number(p.old_purchase_price) || 0,
    new_purchase_price: Number(p.purchase_price) || Number(p.old_purchase_price),
...
function calculateNewPrices() {
  ...
  selectedProducts.value.forEach(product => {
    if (!product.individual_new_price) {
      ...
      product.new_purchase_price  = isIncrease ? base + changeValue : Math.max(0, base - changeValue);
    }
  });
}
...
const hasManualPrice = computed(() => selectedProducts.value.some(
  p => p.individual_new_price && Number(p.new_purchase_price) >= 0 && Number(p.new_purchase_price) !== Number(p.old_purchase_price)
));
...
async function formSubmit() {
  ...
  toast.add({ severity: 'success', summary: 'Success Message', detail: 'Purchase price change updated successfully.', life: 3000 });
...
<template>
<div class="p-4">
  <PageTitle title="Update Purchase Price Change">
    <template #titleButtons>
      <div class="flex gap-x-2 items-center">
        <BaseButton icon="fa fa-chevron-left" label="Back" severity="secondary" @click="changeRoute('/purchase_price_change')" />
      </div>
    </template>
  </PageTitle>
            ...
            <table class="w-full text-sm border-collapse">
              ...
              <tbody>
                <tr v-for="product in selectedProducts" :key="product.id" class="border-b hover:bg-gray-50">
                    ...
                    <td class="py-2 text-right">{{ formatPrice(product.old_purchase_price) }}</td>
