...
async function syncFromCloud() {
    await useCounter.addCounter({updated_by: JSON.parse(localStorage.getItem('user')).id});
    if (useCounter.error.length) {
        useCounter.error.forEach((msg) => {
            toast.add({
                severity: 'error',
...
<template>
    <div class="p-4">
        <PageTitle title="Counter List">
            <template #titleButtons>
                <div class="flex gap-x-2 items-center">
                    <BaseButton v-if="usePermission.can('Counter', 'Create')" icon="fa fa-cloud-arrow-down" label="Sync from Cloud"
        ...
        <DataTable :columns="columns" :rows="filteredRows" :editPath="'Update Counter'"
            ...
            :isEdit="!usePermission.can('Counter', 'Update')" :isDelete="!usePermission.can('Counter', 'Delete')" filename="Counter">
            <template #filters>
