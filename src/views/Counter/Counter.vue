<script setup>
    ...
    async function deleteHandle(id) {
        ...
        if(useCounter.error) {
            toast.add({ severity: 'error', summary: 'Error Message', detail: useCounter.error, life: 3000 });
            return
        }
            ...
            <template #titleButtons>
                <div class="flex gap-x-2 items-center">
                    <BaseButton 
                        ...
                        label="Create" 
                        severity="primary" 
                        @click="changeRoute('/counter/create')"  />
                </div>
            </template>
        </PageTitle>
        <DataTable 
            ...
            :rows="filteredRows" 
            :pageSize="5" 
            :editPath="'Update Counter'" 
            :isLoading="useCounter.loading" 
            @delete="deleteHandle"
            :defaultSort="{key: 'created_at', order: 'desc'}"
            :isEdit="!usePermission.can('Counter', 'Update')"
            :isDelete="!usePermission.can('Counter', 'Delete')"
        >
            <template #filters>
