<script setup>
    ...
    async function formSubmit() {
        ...
        if(usePaymentMethod.error) {
            Object.values(usePaymentMethod.error).forEach((err) => {
                err.forEach((msg) => {
        ...
        <PageTitle title="Update Payment Method">
            <template #titleButtons>
                <div class="flex gap-x-2 items-center">
                    <BaseButton icon="fa fa-chevron-left" label="Back" severity="secondary" @click="changeRoute('/payment_method')"  />
                </div>
            </template>
        </PageTitle>
        <!-- Form Section -->
            ...
            <template #cardElements>
                ...
                <div class="flex justify-end mt-4">
                    ...
                    <BaseButton
                        ...
                        :icon="usePaymentMethod.loading? 'fa fa-spinner' : 'fa fa-floppy-disk'" 
                        severity="primary" 
                        @click="formSubmit" 
                        :disabled="usePaymentMethod.loading"  
                    />
                </div>
            </template>
        </BaseCard>
